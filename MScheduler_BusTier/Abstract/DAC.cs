...
//********************************************************************************
//  Definition : The DAC class is intended to encapsulate high performance, 
//               scalable best practices for common uses of SqlClient.
//               It should simplify access to the db via ADO.NET
//
//   NOTE: All Executes are overloaded identically to standardize the interface
//
//       This component is based on the Microsoft SQL Application block
//
//  Properties :
//       
//
//  Methods:
//       ExecuteNonQuery - Runs an action query with no return                   - Overloaded
//       ExecuteDataset  - Runs a query or SP that returns an ADO type DataSet   - Overloaded
//       ExecuteReader   - Runs a query or SP that returns an ADO type DataReader- Overloaded
//       ExecuteScalar   - Runs a query or SP that returns a 1x1 resultset       - Overloaded
//       ExecuteXmlReader- Runs a query or SP that returns XML                   - Overloaded
//
//   Author: Steve Dolan
//
//  Revision:
...
public class DAC2 {
    ...
    //instances from being created with "new DAC()".
    private DAC2() {
    }
    //New
    ...
    public static int ExecuteNonQuery(string connectionString, string spName, params object[] parameterValues) {
        ...
        if ((parameterValues != null) & parameterValues.Length > 0) {
            ...
            commandParameters = DACParameterCache.GetSpParameterSet(connectionString, spName);
    ...
    public static int ExecuteNonQuery(SqlConnection connection, string spName, params object[] parameterValues) {
        ...
        if ((parameterValues != null) & parameterValues.Length > 0) {
            //pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
            commandParameters = DACParameterCache.GetSpParameterSet(connection.ConnectionString, spName);
    ...
    // This method provides no access to output parameters or the stored procedure's return value parameter.
    // e.g.:  
    // Dim result As Integer = DAC2.ExecuteNonQuery(trans, "PublishOrders", 24, 36)
    // Parameters:
    // -transaction - a valid SqlTransaction 
    // -spName - the name of the stored procedure 
    // -parameterValues - an array of objects to be assigned as the input values of the stored procedure 
    ...
    public static int ExecuteNonQuery(SqlTransaction transaction, string spName, params object[] parameterValues) {
        ...
        if ((parameterValues != null) & parameterValues.Length > 0) {
            //pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
            commandParameters = DACParameterCache.GetSpParameterSet(transaction.Connection.ConnectionString, spName);
    ...
    // the connection string. 
    // e.g.:  
    // Dim ds As DataSet = DAC2.ExecuteDataset("", commandType.StoredProcedure, "GetOrders")
    // Parameters:
    // -connectionString - a valid connection string for a SqlConnection
    // -commandType - the CommandType (stored procedure, text, etc.)
    // -commandText - the stored procedure name or T-SQL command
    ...
    // this enum is used to indicate whether the connection was provided by the caller, or created by DAC, so that
    // we can set the appropriate CommandBehavior when calling ExecuteReader()
    private enum SqlConnectionOwnership {
        //Connection is owned and managed by DAC
        Internal,
    ...
    // -commandText - the stored procedure name or T-SQL command 
    // -commandParameters - an array of SqlParameters to be associated with the command or 'null' if no parameters are required 
    // -connectionOwnership - indicates whether the connection parameter was provided by the caller, or created by DAC 
    ...
    //        Output = ExecuteReader(CommandType.StoredProcedure, spName, commandParameters)
    ...
    public static string CleanSQLString(string textToSave) {
        if (textToSave == null) {
            return null;
        }
...
//********************************************************************************
//  Definition : The DACParameterCache class is intended to cache SQL Parameters
//
//
//       This component is based on the Microsoft SQL Application block
//
...
//
//  Methods:
//       CacheParameterSet       - Adds parameter array to the cache
//       GetCachedParameterSet   - Retrieves a parameter array from the cache
//       GetSpParameterSet       - Discover Param types from the DB and cache
//
//
//   Author: Steve Dolan
//
...
public sealed class DACParameterCache {

    #region "private methods, variables, and constructors"


    //Since this class provides only static methods, make the default constructor private to prevent 
    //instances from being created with "new DACParameterCache()".
    private DACParameterCache() {
    }
    //New 
    ...
    //searches for index in SqlParameter array
    // Oct 31 2006 krb: Wrote original function.
    public static int FindParameterIndex(ref SqlParameter[] parameters, string parameterName) {

        int i = 0;

        for (i = 0; i <= parameters.Length - 1; i++) {
            if (parameters[i].ParameterName.ToUpper() == parameterName.ToUpper()) {
    ...
    public static SqlParameter[] GetSpParameterSet(string connectionString, string spName, bool includeReturnValueParameter) {
        ...
        if (includeReturnValueParameter == true) {
            hashKey = connectionString + ":" + spName + ":include ReturnValue Parameter";
        }
        else {
...
//********************************************************************************
//  Definition : The DACConnect class is used to set up the connection string
//               used in the DAC Class. It does NOT create a connection object
//
//  Properties :
//      DBType   - String -  Type of database ie app, dev, THR, loc etc  
//      DBServer - String -  Server location ie local, 172......   
//      DBApp    - String -  Name of the Database. DBType + DBName = connection database 
//      DBUser   - String -  Generic USer ID for the Application
//      DBPass   - String -  Generic Password for the Application
//      DBName   - String -  The Connection name the Database ie DBType + DBName 
//      DBConn   - String -  The Full Connection String
//  Methods:
//
//
//   Author: Steve Dolan
...
//
//*********************************************************************************
public class DACConnect {

    #region "ClassPropertyVars"
    ...
    public string DBType {
        get { return msdbType.ToString(); }
        set { msdbType = value.ToString(); }
    }
    public string DBServer {
        get { return msdbServer.ToString(); }
        set { msdbServer = value.ToString(); }
    }
    public string DBApp {
        get { return msdbName.ToString(); }
        set { msdbName = value.ToString(); }
    }
    public string DBUser {
        get { return msUser.ToString(); }
        set { msUser = value.ToString(); }
    }
    public string DBPass {
        get { return msPassword.ToString(); }
        set { msPassword = value.ToString(); }
    }
    public string DBName {
        get { return msdbType.ToString() + msdbName.ToString(); }
    }
    public string DBConn {
        get {
            string sConn = "data source=";
            sConn += msdbServer.ToString();
            sConn += " ;initial catalog=" + DBName;
            sConn += " ;User ID=" + msUser.ToString();
            sConn += " ;password=" + msPassword.ToString() + ";";
            return sConn.ToString();
        }
    }
